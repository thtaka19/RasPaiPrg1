...
void set_vector_table(void){
	extern void *__start_vector;
	extern void *__end_vector;

	volatile unsigned int *vec = 0;
	volatile unsigned int *p;
	volatile unsigned int *s = (unsigned int *)&__start_vector;//  _initial_vector_start
	volatile unsigned int *e = (unsigned int *)&__end_vector;//  _initial_vector_end
...
void print_vector_table(unsigned int *srt){
	extern void *__start_vector;
	extern void *__end_vector;
	...
	volatile unsigned int *e = (unsigned int *)&__end_vector;//  _initial_vector_end
	vec = srt;
	m_serialPutStr("Vector table print\r\n");

	m_serialPutStr("Addr : Hex\r\n");
	for (i = 0; i < 32; i++) {
//		*vec = *p;
//		printf("0x%02x : 0x%08x\r\n",vec,*vec);
		putxval0((unsigned long)vec,8);
m_serialPutStr(":");
		putxval0((unsigned long)*vec,8);
m_serialPutStr("\r\n");	
vec++;	
...
/////   Vecter address Set 2018 TT
	...
	//SVC_PERROR(syslog_msk_log(LOG_UPTO(LOG_INFO), LOG_UPTO(LOG_EMERG)));// 2018_06 TT
	syslog(LOG_NOTICE, "Sample program starts (exinf = %d).", (int_t) exinf);
m_serialPutStr("Test Ver3 ---");//m_serialGetChar  m_serialPutChar  m_serialInit()
	/*
	 *  シリアルポートの初期化
	 *
	 ...
	 */
m_serialInit();
m_serialPutStr("Vector Table ox000=~ \r\n");
print_vector_table(0);
m_serialPutStr("Vector Table ox8000=~ \r\n");
print_vector_table(0x8000);
