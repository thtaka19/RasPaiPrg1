...
//			rpiAux->MU_IER_REG=0x01;	//Enable Receive Only(1)
			rpiAux->MU_iir_REG=0x01;	//Enable Receive Only(1)???   0x02?? TT
...
//			rpiAux->MU_IER_REG=0x03;	//Enable R(1)&T(2)
			rpiAux->MU_iir_REG=0x01;	//Enable R(1)&T(2) TT
		//	putChar1('?');	// not mini UART interrupt ?
		}
	} else {
		m_serialPutChar('?');	// not mini UART interrupt ?
...
//	rpiAux->MU_IER_REG=0;	//disable interrupt
	rpiAux->MU_iir_REG=0;	//disable interrupt
...
//	rpiAux->MU_IIR_REG=0xC6;	// 0xC6 FIFO Enable/Clear	
	rpiAux->MU_ier_REG=0xC6;	// 0xC6 FIFO c:Enable/ 6:Clear
...
//	rpiAux->MU_iir_REG=0x01;	//Enable 2:Receive Interruptrrupi 1:trancemit Interrupt 1+2= 0x03 TT
	rpiAux->MU_iir_REG=0x00;	// No Interrupt
...
void enQueT(int data){ // For Interrupt Send( Transfer)
	...
	struct rpiIrqController *rpiIRQController = 
		(struct rpiIrqController *)RPI_INTERRUPT_CONTROLLER_BASE;
	struct rpiAux* rpiAux= RPI_AUX_BASE;
 	rpiAux->MU_iir_REG=0x00;// mini_UART Interrupt Disable
	temp =inpT;
	...
	if(temp ==outpT) {   // 2017 TT
	  ...
	  rpiAux->MU_iir_REG=0x01;	//Enable Receive Interrupt 0x03	TT
	  return;  
	}   
...
void enQueR(int data){ // For Interrupt Receive
	...
	if(temp ==outpR) {   // 2017 TT
	  ...
	  rpiAux->MU_iir_REG=0x01;	//Enable Receive Interrupt 0x03	
	  return;  
	}     
	*inpR = data;
	inpR = temp;
...
char *m_serialGetStr(char *s,int len){
	int i=1;
	int c;
	do{
		c = m_serialGetChar();
