...
void set_vector_table(void){
	...
	printf("Addr : Hex\n");
	vec++;// Correct 2019 TT
	for (p = s+1; p < e; p++) {// Correct 2019 TT
		*vec = *p;
...
int main(void){
	...
	//disableDataCache();
	disableMMU();// U-Boot need this May be 2019 TT
	//disable_IRQ();
	rpi_init();
	
	// ベクタテーブルセット
	set_vector_table();
    writeVectorBaseRegister(0x0000);// U-Boot need this May be 2019 TT
	setLowVector();// U-Boot need this May be 2019 TT
	// すべての割り込み不許可
